...
**v0.11.0 shipped — see CHANGELOG for the current patch.** Full fiscal lifecycle covered:
...
- **v0.10.0** — `substituir()` emits only the DPS with `<subst>` (the 105102 event is generated by the Sistema Nacional; no contribuinte-side event, no rollback — `SubstituirResult` has the same shape as `EmitirResult`); fail-fast emission guards (`totTrib` choice per regime, CPF/CNPJ DV, E0595/E0600/E0580 etc.); `validateCnpj` accepts alphanumeric CNPJ (IN RFB nº 2.229/2024, DV via ASCII−48).
- **v0.10.1** — **NT 008/2026**: official DANFSe generation API suspended on 2026-08-03. `gerarDanfse` default flipped to `'local'`; `'auto'`/`'online'` strategies and `consultarDanfse` are deprecated (each use logs `danfse.online.deprecated`) — **do not flip the default back to `'auto'`.** Pending: conform the local pdfkit renderer to the NT 008 single national layout (tracked in `specs/standards-watch.md`).
- **v0.11.0** — adopted XSD bundle **20260727** (CNPJ alfanumérico, in produção since 2026-08-10): `schemas/1.01/` + regenerated WASM validator accept `[0-9A-Z]{14}` CNPJs in `TSCNPJ`, DPS/evento Ids, and chaves; `buildDpsId`/`buildEventoPedidoId` and the chave/Id pre-validation regexes (`fetchByChave`, `fetchDpsStatus`, DANFSe, `NfseClientFake`) mirror the new patterns; `TSSerieDPS` officially fixed — séries 90000–99999 now rejected.
...
- **Canonical XSDs**: `schemas/1.01/` — the 10 official XSDs (RTC v1.01, **bundle 20260727** — CNPJ alfanumérico patterns, in produção since 2026-08-10). `scripts/generate-schemas.mjs` inlines them into `src/nfse/_rtc-schemas.generated.ts` for the WASM validator; rerun it after any schema edit. `schemas/1.00/` is the obsolete 2022 base, kept for reference only. The old libxml-compat deviation on `TSSerieDPS` is gone: the 20260727 bundle officially fixed the pattern to `[0-9]{1,4}|[0-8][0-9]{4}` (séries 90000–99999 invalid) — don't re-introduce the local patch.
