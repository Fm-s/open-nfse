...
| `substituir`          | `(params: SubstituirParams) → SubstituirResult`     | Emite nova DPS com `<subst>`; sistema gera o 105102. Discriminado `ok` / `retry_pending` |
...
|----------------------------|-------------------------------------------------------------------------------------|----------------------------------------------------------|
| `buildDps`                 | `(params: BuildDpsParams) → DPS`                                                    | Monta DPS de ~10 campos; `totTrib` por regime (MEI `indTotTrib` / ME-EPP `pTotTribSN` / Não Optante `vTotTrib`·`pTotTrib`); valida DV CPF/CNPJ |
...
| `SubstituirResult` (discriminated) | `{ status:'ok', novaNfse }` \| `{ status:'retry_pending', pending, error }` — sistema gera o 105102; transiente → RetryStore; sem rollback |
