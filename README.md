...
**⚠️ Radar do padrão (ago/2026):** desde **03/08/2026** o preenchimento dos grupos **IBS/CBS** é obrigatório na NFS-e Nacional, com as regras de validação ativas — no **leiaute já vigente** (NT 004 + `tpRetPisCofins` da NT 007), que a lib já cobre por completo. DPS sem o grupo `IBSCBS` é rejeitada pela SEFIN (optantes do Simples Nacional só a partir de 2027). **CNPJ alfanumérico na DPS entra em produção em 10/08/2026**, desacoplado da NT 009: o CGNFS-e publicou em Produção Restrita o bundle XSD `esquemas-nfse-rtc-v1-01-20260727` (mesma versão 1.01; muda os padrões de `TSCNPJ`, Ids de DPS/NFS-e/eventos e chaves para aceitar `[0-9A-Z]` na inscrição) — a adoção do bundle na lib está planejada para a janela de 10/08 (diff mapeado em [`specs/standards-watch.md`](./specs/standards-watch.md)); `validateCnpj` já aceita o **CNPJ alfanumérico** (IN RFB nº 2.229/2024). A **NT 009/2026** (leiaute V1.04.00: notas de crédito/débito IBS/CBS, `vAjusteBC`, `gPgtoVinc`, locação) segue **adiada** — oficialmente fora de agosto/2026; cronograma ainda sem data. **NT 008/2026** (v1.02, 14/07/2026): a API oficial de geração do DANFSe foi **suspensa em 03/08/2026** — desde a v0.10.1 o default de `gerarDanfse` é `'local'` e o caminho online (`'auto'`/`'online'`/`consultarDanfse`) está deprecated; a adequação do renderer local ao novo leiaute nacional multi-tributo da NT 008 está no roadmap. Acompanhamento e roteiro de verificação em [`specs/standards-watch.md`](./specs/standards-watch.md).
